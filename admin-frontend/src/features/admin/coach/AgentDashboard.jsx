...
export function AgentPayments({ payments, userId, lang }) {
  ...
  return <section className="dashboard-panel agent-section"><div className="section-heading"><div><h2>Paiements</h2><p>Validation des paiements en espèces (CASH).</p></div><Link to="/transport/operations" className="button secondary sm">Operations</Link></div>{error && <div className="alert alert-danger" role="alert">{error}</div>}
    {!pending.length && !payments.validatedToday?.length ? <EmptyState title="Aucun paiement" description="Aucun paiement reel a traiter ou valide aujourd hui." /> : <><div className="agent-payment-summary">{pending.length} paiement(s) cash en attente de validation au guichet.</div>{pending.length > 0 && <div className="table-responsive"><table className="data-table"><thead><tr><th>Reservation</th><th>Canal</th><th>Montant</th><th>Methode</th><th>Reference</th><th>Date</th><th>Action</th></tr></thead><tbody>{pending.map((item) => <tr key={item.id}><td>{item.reservation?.reservationCode || '-'}</td><td>Cash / Agence</td><td>{money(item.amount, item.currency, lang)}</td><td>{item.method || 'CASH'}</td><td>{item.reference || '-'}</td><td>{dateTime(item.createdAt, lang)}</td><td><Button size="sm" loading={validate.isPending && validate.variables === item.id} onClick={() => validate.mutate(item.id)}>Valider cash</Button></td></tr>)}</tbody></table></div>}{!!payments.validatedToday?.length && <div className="agent-subsection"><h3>Valides aujourd hui</h3><div className="table-responsive"><table className="data-table"><thead><tr><th>Reservation</th><th>Montant</th><th>Methode</th><th>Reference</th><th>Valide par</th><th>Date</th></tr></thead><tbody>{payments.validatedToday.map((item) => <tr key={item.id}><td>{item.reservation?.reservationCode || '-'}</td><td>{money(item.amount, item.currency, lang)}</td><td>{item.method || '-'}</td><td>{item.reference || '-'}</td><td>{item.validatedBy ? `${item.validatedBy.firstName} ${item.validatedBy.lastName}` : '-'}</td><td>{dateTime(item.validatedAt, lang)}</td></tr>)}</tbody></table></div></div>}</>}
  </section>
}
