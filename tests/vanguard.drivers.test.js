...
test('vanguard drivers CRUD', async () => {
  ...
  const create = await request('POST', '/api/drivers', {
  ...
  const driverId = create.data.data.driver.id;

  const getOne = await request('GET', `/api/drivers/${driverId}`, null, adminToken);
  assert.equal(getOne.status, 200);

  const list = await request('GET', '/api/drivers', null, adminToken);
  assert.equal(list.status, 200);

  const update = await request('PUT', `/api/drivers/${driverId}`, { firstName: 'Jane' }, adminToken);
  assert.equal(update.status, 200);

  const del = await request('DELETE', `/api/drivers/${driverId}`, null, adminToken);
