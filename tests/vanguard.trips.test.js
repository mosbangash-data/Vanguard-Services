...
test('vanguard trips CRUD', async () => {
  ...
  const busRes = await request('POST', '/api/buses', { departmentId: department.id, plateNumber: `TR-${Date.now()}`, brand: 'Test', model: 'Model', seats: 20 }, adminToken);
  assert.equal(busRes.status, 201);
  const busId = busRes.data.data.bus.id;

  const scheduleRes = await request('POST', '/api/schedules', { departmentId: department.id, routeId, busId, departureTime: '08:00', availableDays: ['MON'], price: '10.00' }, adminToken);
  ...
  const list = await request('GET', '/api/trips', null, adminToken);
  assert.equal(list.status, 200);

  const update = await request('PUT', `/api/trips/${tripId}`, { status: 'IN_PROGRESS' }, adminToken);
  assert.equal(update.status, 200);

  const del = await request('DELETE', `/api/trips/${tripId}`, null, adminToken);
