...
test('vanguard destinations CRUD', async () => {
  ...
  const create = await request('POST', '/api/destinations', {
  ...
  const id = create.data.data.route.id;

  const getOne = await request('GET', `/api/destinations/${id}`, null, adminToken);
  assert.equal(getOne.status, 200);

  const list = await request('GET', '/api/destinations', null, adminToken);
  assert.equal(list.status, 200);

  const update = await request('PUT', `/api/destinations/${id}`, { description: 'Test route' }, adminToken);
  assert.equal(update.status, 200);

  const del = await request('DELETE', `/api/destinations/${id}`, null, adminToken);
